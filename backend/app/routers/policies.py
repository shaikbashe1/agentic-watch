...
from ..services.auth_service import get_current_user, get_workspace_from_api_key
...
def create_policy(policy: PolicyCreate, current_user: tenant.User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def update_policy(id: str, policy: PolicyUpdate, current_user: tenant.User = Depends(get_current_user), db: Session = Depends(get_db)):
    return policy_service.update_policy(db, current_user.workspace_id, id, policy)


@router.delete("/{id}")
def delete_policy(id: str, current_user: tenant.User = Depends(get_current_user), db: Session = Depends(get_db)):
