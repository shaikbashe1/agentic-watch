...
export function useTokenTimeseries() {
  return useQuery({
    queryKey: ["timeseries-tokens"],
    queryFn: async () => {
      const { data } = await api.get<{date: string, tokens: number}[]>("/analytics/timeseries/tokens");
      return data;
    },
...
export function useCostTimeseries() {
  return useQuery({
    queryKey: ["timeseries-costs"],
    queryFn: async () => {
      const { data } = await api.get<{date: string, cost: number}[]>("/analytics/timeseries/costs");
      return data;
    },
    refetchInterval: 15_000,
  });
...
export function useLatencyTimeseries() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{time: string, latency: number}[]>("/analytics/timeseries/latency");
      return data;
    },
    refetchInterval: 15_000,
  });
...
export function useTrace(traceId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<any[]>(`/analytics/traces/${traceId}`);
      return data;
    },
    enabled: !!traceId,
  });
